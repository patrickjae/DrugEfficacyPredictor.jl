...
# > perl create_random_predictions.pl /path/to/training/data /path/to/test/data /path/to/outputdir/ [#randpredictions]
# default number of random predictions is 100
#################################################################
use File::Spec;
######
# global variables
my $trainingdata = $ARGV[0];
my $testdata = $ARGV[1];
my $outdir = $ARGV[2];
my $number_of_random = $ARGV[3] || 100;
...
# randomly into the ranked training cell lines. 
    ...
    my $outfile = File::Spec->catfile($outdir,"sc1_random_" . $i . ".txt");
