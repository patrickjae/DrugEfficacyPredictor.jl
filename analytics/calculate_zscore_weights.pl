...
# > perl calculate_zscore_weights.pl /path/to/randpredictions/ /path/to/pooled/std /path/to/test/data /path/to/ground/truth /path/to/zscore/outfile
#################################################################
...
my $rand_dir = $ARGV[0];
my $sdfile = $ARGV[1];
my $testdatafile = $ARGV[2];
my $groundtruth = $ARGV[3];
my $outfile = $ARGV[4];
...
my @drugs = ();
my %preds = ();
open(FILE, "$groundtruth") || die "Please enter a file to score\n";
while(<FILE>) {
    chomp;
    my ($cellLine, @vals) = split(/\,/, $_);
...
while(my $file = readdir(DIR)) {
    ...
    my %randpreds = ();
    my @randdrugs = ();
    $filename = File::Spec->catfile($rand_dir,$file);
    open(FILE, "$filename") || die "random file not found; $filename\n";
    while(<FILE>) {
	chomp;
	my ($cellLine, @vals) = split(/\,/, $_);
	if ($cellLine eq 'DrugAnonID') {
